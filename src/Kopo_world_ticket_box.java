...
public class Kopo_world_ticket_box {
	...
	int check_ticket_type() {
		...
		do {
			data_variable.ticket = sc.nextInt();
			switch (data_variable.ticket) {
				case 1: case 2:
					System.out.printf("\n");
					break;
				default :
		...
		} while (data_variable.ticket!=1 && data_variable.ticket!=2); 
		return data_variable.ticket;
	}
	
	int check_day_type() {
		System.out.printf("이용시간을 선택 하세요.\n1. 1일권 \n2. AFTER4(16시 이후) \n->");
		do{
			data_variable.dayTicket = sc.nextInt();
			switch (data_variable.dayTicket) {
				case 1: case 2:
					System.out.printf("\n");
					break;
				default :
		...
		}while (data_variable.dayTicket!=1 && data_variable.dayTicket!=2);
		return data_variable.dayTicket;
	}
	...
	int check_person_ID() {
		...
		do{
			...
			date = Integer.parseInt(personID.substring(4,6));
			gender =Integer.parseInt(personID.substring(6,7));
			...
			if (four_digit_year % 4 == 0 && four_digit_year % 100 != 0 || four_digit_year%400 == 0) {
				lun_Date = date;
				if (lun_Date < 1 || lun_Date > static_variable.MONTH_LUN[month-1]) {
					System.out.printf("출생일 다시 확인 해 주세요.\n");
				}
			} else {
				nor_Date = date;
				if (nor_Date < 1 || nor_Date > static_variable.MONTH_NOR[month-1]) {
					System.out.printf("출생일 다시 확인 해 주세요.\n");
				}				
			}
			if (gender  < 1 && gender > 4) {
				...
				|| (lun_Date < 1 || lun_Date > static_variable.MONTH_LUN[month-1]) 
				&& (nor_Date < 1 || nor_Date > static_variable.MONTH_NOR[month-1]) 
				|| (gender  <1 || gender > 4) 
				|| (personID.length() != 13) 
				|| (year > current_year%100 && year < current_year%100+18));
		return gender;
	}
...
//		orderArr[indexOfArray][static_variable.SUM_PRICE] = grandPrice;
	...
	void caculate_data(){
		...
		calculate_ticket_price (data_variable.ticket, data_variable.dayTicket, data_variable.generation);
		adjust_advantage(data_variable.advantage, data_variable.price, data_variable.qty, gender);	
		input_data_in_Arr(data_variable.ticket, data_variable.dayTicket, data_variable.advantage, data_variable.disAdvantage, data_variable.generation, data_variable.num, data_variable.disNum, data_variable.totalPrice, data_variable.totalDiscountPrice, data_variable.grandPrice);
	...
	void print_total_order(){
		...
		for (int roundOfOrder = 0; roundOfOrder<make_list.size(); roundOfOrder++){//할인 티켓 출력 
			...
			switch (make_list.get(roundOfOrder).ticket) {
				case 1 :	
			...
			switch (make_list.get(roundOfOrder).dayTicket) {
				case 1 :
